      ...
      <div className="bg-white border border-amber-100 rounded-2xl p-6 shadow-sm">
        ...
        <div className="space-y-4">
          <div
            ...
            style={{ background: "#1a1200", border: "2px solid #f5c842" }}
          >
            <div
            ...
            {[
              {
                step: 1,
                title: "마인크래프트 실행",
                desc: "Java Edition 마인크래프트를 실행하세요. 서버 권장 버전을 확인하세요.",
              },
              {
                step: 2,
                title: "멀티플레이 선택",
                desc: "타이틀 화면에서 '멀티플레이' 버튼을 클릭하세요.",
              },
              {
                step: 3,
                title: "서버 추가",
                desc: "'서버 추가' 버튼을 클릭하고 위의 IP 주소를 입력하세요.",
              },
              {
                step: 4,
                title: "접속!",
                desc: "서버가 목록에 추가되면 더블클릭 또는 '서버에 참가' 버튼으로 접속하세요.",
              },
            ].map((s) => (
              <div
                key={s.step}
      ...
      <div className="bg-green-50 border border-green-200 rounded-2xl p-4">
        ...
        <ul className="mt-2 space-y-1">
          {[
            ...
            <li
              key={item}
              className="text-green-700 flex items-start gap-1.5"
              style={{ fontSize: "13px" }}
            >
              ...
              {commands.map((c) => (
                ...
                >
                  <td className="px-4 py-3">
                    <code
                      className="bg-slate-100 text-slate-700 rounded-lg px-2 py-0.5"
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`${catColors[c.cat] ?? "bg-slate-100 text-slate-600"} rounded-full px-2 py-0.5`}
...
function FaqContent() {
  const faqs = [
    ...
    {
      q: "광물변환은 뭔가요?",
      a: "/광물변환 활성화 시 광물들이 드롭되지 않는 대신, 크리스탈 확률이 상승합니다. 태양열의 경우, 태양열 효과가 강화된 비율만큼 크리스탈 확률이 증가해요. 특성 효과로 인해 드롭되는 광물은 여전히 나올 수 있어요.",
    },
    {
      q: "지렁이는 어떻게 얻나요?",
      a: "수확 특성의 경우 지렁이를 더 많이 획득할 수 있어요. 지렁이는 섬세한 손길 인챈트가 있을 때만 드롭되고, 행운 수치가 높을수록 확률이 올라가요! 수확 스킬 '파브르'로도 지렁이 획득 확률을 높일 수 있어요.",
    },
    {
      q: "산삼씨앗은 어디서 얻나요?",
    ...
    },
    {
      ...
      a: "윈도우 검색창에 '고정 키' 검색 → 첫 번째 클릭 → 고정키 사용을 '끔'으로 변경하면 해결돼요! 낚시 미니게임에서 쉬프트를 자유롭게 사용할 수 있게 됩니다.",
    },
    {
      q: "커스텀 작물이 자라지 않아요.",
    ...
    {
      q: "부계정을 사용해도 되나요?",
      a: "같은 아이피에 두 명 이상의 유저가 접속하면 부계정으로 처리될 수 있어요. 가족과 함께 플레이한다면 반드시 '가족계정 신청 게시판'에 미리 신청하세요!",
    },
    {
      q: "암시장이란 무엇인가요?",
      a: "암시장은 특정 랭크 이상(12등급)에서만 /암시장확인 명령어로 위치를 알 수 있는 비밀 상점이에요. 암시장 위치나 시세를 전체채팅/스폰지역채팅/확성기로 발설하면 처벌받을 수 있어요!",
    },
    {
      q: "재화를 잃었을 때 복구받을 수 있나요?",
      a: "복구 요청 시 물증이 반드시 필요해요. 영상(날짜가 확인되는), 방송 리플레이, 또는 1주일 이내 날짜 형식의 사진 2장+본인 인증이 필요해요. 사기나 개인 과실로 인한 손실은 복구되지 않아요.",
    },
  ];
  ...
  return (
    <div className="space-y-3">
      {faqs.map((faq, i) => (
        <details
          key={i}
          className="group bg-white border border-amber-100 rounded-2xl overflow-hidden shadow-sm"
        >
  ...
  const rules = [
    {
      no: "1",
      title: "비인가 프로그램 금지",
      desc: "매크로(블럭 설치·파괴 오토마우스, 잠광 매크로 제외), 핵 등 비인가 프로그램 사용 금지",
      icon: "🤖",
    },
    {
      no: "2",
      title: "플라이 무단침입 금지",
      desc: "플라이를 이용하여 다른 팜에 무단침입하는 행위 금지",
      icon: "✈️",
    },
    {
      no: "3",
      title: "부계정 금지",
      desc: "같은 아이피로 여러 계정 사용 금지 (가족은 가족계정 신청 필수)",
      icon: "👥",
    },
    {
      no: "4",
      title: "사기 금지",
      desc: "거래가 성립된 후 대가를 지불하지 않는 사기 행위 엄금",
      icon: "🚨",
    },
    {
      no: "5",
      title: "절도 및 테러 금지",
      desc: "타인의 섬 아이템 무단 취득, 블럭 파손, 고의 피해 행위 금지",
      icon: "🔒",
    },
    {
      no: "6",
      title: "욕설 및 비매너 채팅 금지",
      desc: "비속어, 비하 발언, 도배, 타서버 언급, 반말 등 비매너 채팅 금지",
      icon: "💬",
    },
    {
      no: "7",
      title: "멀티팜 금지",
      desc: "여러 팜을 운영하며 재화를 비정상적으로 한 곳에 몰아주는 행위 금지",
      icon: "🏭",
    },
    {
      no: "8",
      title: "시세 파괴 금지",
      desc: "캐시나 아이템을 비정상적인 가격으로 거래하여 시세에 혼란을 야기하는 행위 금지",
      icon: "📉",
    },
    {
      no: "9",
      title: "무고죄 금지",
      desc: "죄가 없는 유저를 고의로 신고하는 행위 금지",
      icon: "⚖️",
    },
    {
      no: "10",
      title: "현금 거래 금지",
      desc: "서버 간 현금/현물 거래 시 아이템 초기화 및 엄격한 처벌",
      icon: "💸",
    },
  ];
    ...
    <div className="space-y-4">
      <div className="bg-red-50 border border-red-200 rounded-2xl p-4">
        <p
          ...
          style={{ fontSize: "13px", lineHeight: 1.7 }}
        >
          ⚠️ 규칙 위반 시 경고, 아이템 몰수, 임시/영구 정지 등의 처벌을 받을 수
          있어요. 자세한 처벌 기준은{" "}
          <Link to="/law" className="underline font-bold">
            법전
          </Link>
          에서 확인하세요.
        </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {rules.map((r) => (
          <div
            key={r.no}
            className="bg-white border border-amber-100 rounded-2xl p-4 shadow-sm flex items-start gap-3"
          >
            <span className="text-2xl flex-shrink-0">{r.icon}</span>
            <div>
              <div className="flex items-center gap-2 mb-0.5">
                <span
                  className="bg-red-100 text-red-600 rounded-full px-1.5 py-0.5"
    ...
    <div className="space-y-6">
      <div className="bg-red-50 border border-red-200 rounded-2xl p-4">
        <p
          ...
          style={{ fontSize: "13px", lineHeight: 1.7 }}
        >
          ⚠️ 금지 아이템 목록은 서버 업데이트에 따라 변경될 수 있어요. 인게임
          명령어 또는 운영진에게 문의하여 최신 목록을 확인하세요.
        </p>
      </div>
      <div className="bg-white border border-amber-100 rounded-2xl p-6 shadow-sm text-center">
        ...
        <p
          ...
          style={{ fontSize: "13px", lineHeight: 1.7 }}
        >
          금지 아이템 목록은 운영진에 의해 관리되며,
          <br />
          디스코드 공지 또는 인게임 공지를 통해 안내됩니다.
          <br />
        ...
        {[
          {
            icon: "💬",
            title: "채팅 시스템",
            desc: "전체채팅(기본), 지역채팅, 귓속말(/귓말 닉네임) 등을 지원해요. 확성기로 전체 공지도 가능해요 (유료).",
          },
          {
            icon: "📦",
            title: "인벤토리",
            desc: "개인 인벤토리 외에 엔더상자(/엔더상자), 창고(/창고 1~10)를 사용할 수 있어요. 등급별로 창고가 개방됩니다.",
          },
          {
            icon: "💰",
            title: "경제 시스템",
            desc: "재화(원화)로 아이템을 사고팔 수 있어요. 경매장, 개인 상점, NPC 상점을 이용할 수 있습니다.",
          },
          {
            icon: "📮",
            title: "우편함",
            desc: "이벤트 보상, GM 지급 아이템 등은 우편함으로 전달돼요. /우편 명령어로 확인하거나 인벤토리 UI를 통해 확인하세요.",
          },
          {
            icon: "🏝️",
            title: "섬 시스템",
            desc: "자신만의 섬을 개설하고 농장, 광산, 벌목장 등을 운영할 수 있어요. 섬원을 초대하여 같이 운영도 가능해요.",
          },
          {
            icon: "🏪",
            title: "경매장",
            desc: "아이템을 경매장에 등록하여 다른 유저에게 판매할 수 있어요. 세금이 부과될 수 있으며, 고등급에서는 세금이 면제됩니다.",
          },
        ].map((item) => (
          <div
            key={item.title}
