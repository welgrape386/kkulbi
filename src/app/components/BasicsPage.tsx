...
function CommandsContent() {
  ...
  const sortedCommands = [...commands].sort((a, b) => {
    return rankOrder.indexOf(a.req) - rankOrder.indexOf(b.req);
  });
...
function FaqContent() {
  const faqs = [
    ...
    {
      q: "광물변환은 뭔가요?",
      a: "/광물변환 활성화 시 광물들이 드롭되지 않는 대신, 크리스탈 확률이 상승합니다. 태양열의 경우, 태양열 효과가 강화된 비율만큼 크리스탈 확률이 증가해요.",
    },
    {
      q: "지렁이는 어떻게 얻나요?",
        ...
        {[
          {
            icon: "💬",
            title: "채팅 시스템",
            desc: "전체채팅(기본), 지역채팅, 귓속말(/귓말 닉네임) 등을 지원해요.\n확성기로 전체 공지도 가능해요 (유료).",
          },
          {
            icon: "📦",
            title: "인벤토리",
            desc: "개인 인벤토리 외에 엔더상자(/엔더상자), 창고(/창고 1~10)를 사용할 수 있어요.\n등급별로 창고가 개방됩니다.",
          },
          {
            icon: "💰",
            title: "경제 시스템",
            desc: "재화(원화)로 아이템을 사고팔 수 있어요.\n경매장, 개인 상점, NPC 상점을 이용할 수 있습니다.",
          },
          {
            icon: "📮",
            title: "우편함",
            desc: "이벤트 보상, GM 지급 아이템 등은 우편함으로 전달돼요.\n/우편 명령어로 확인하거나 인벤토리 UI를 통해 확인하세요.",
          },
          {
            icon: "🏝️",
            title: "섬 시스템",
            desc: "자신만의 섬을 개설하고 농장, 광산, 벌목장 등을 운영할 수 있어요.\n섬원을 초대하여 같이 운영도 가능해요.",
          },
          {
            icon: "🏪",
            title: "경매장",
            desc: "아이템을 경매장에 등록하여 다른 유저에게 판매할 수 있어요.\n세금이 부과될 수 있으며, 고등급에서는 세금이 면제됩니다.",
          },
        ].map((item) => (
          <div
            key={item.title}
