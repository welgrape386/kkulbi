...
const allSkills = [
  ...
  { lv: 20, name: "일확천농 [작물] [액티브] Ⅰ", type: "액티브", desc: "수확 시 0.1% 확률로 스킬 발동 (스킬 발동 시 농작물 한 세트 드롭). 쿨타임: 3분", cost: "5,000,000원" },
  { lv: 40, name: "파브르 Ⅰ", type: "패시브", desc: "수확 시 지렁이 0.15% 확률로 획득 (모든 작물에서 효과 적용)", cost: "7,000,000원" },
  { lv: 60, name: "빛나는 확률 Ⅰ", type: "패시브", desc: "수확 시 산삼 씨앗 0.03% 확률로 획득 (모든 작물에서 효과 적용)", cost: "10,000,000원" },
  { lv: 80, name: "수확신의 축복 Ⅱ", type: "패시브", desc: "수확 시 경험치 7.5% 추가 획득 (사탕수수에는 작동하지 않음)", cost: "15,000,000원" },
  { lv: 100, name: "드랍더 농작물 Ⅱ", type: "패시브", desc: "수확 시 0.45% 확률로 농작물 추가 드롭 (사탕수수에는 작동하지 않음)", cost: "20,000,000원" },
  { lv: 110, name: "일확천농 [작물] [액티브] Ⅱ", type: "액티브", desc: "수확 시 0.2% 확률로 스킬 발동 (스킬 발동 시 농작물 한 세트 드롭). 쿨타임: 3분", cost: "30,000,000원" },
  { lv: 120, name: "파브르 Ⅱ", type: "패시브", desc: "수확 시 지렁이 0.25% 확률로 획득 (모든 작물에서 효과 적용)", cost: "50,000,000원" },
  { lv: 130, name: "빛나는 확률 Ⅱ", type: "패시브", desc: "수확 시 산삼 씨앗 0.25% 확률로 획득 (모든 작물에서 효과 적용)", cost: "70,000,000원" },
  { lv: 140, name: "수확신의 축복 Ⅲ", type: "패시브", desc: "수확 시 경험치 9% 추가 획득 (사탕수수에는 작동하지 않음)", cost: "100,000,000원" },
  { lv: 150, name: "드랍더 농작물 Ⅲ", type: "패시브", desc: "수확 시 0.7% 확률로 농작물 추가 드롭 (사탕수수에는 작동하지 않음)", cost: "130,000,000원" },
  { lv: 160, name: "일확천농 [작물] [액티브] Ⅲ", type: "액티브", desc: "수확 시 0.25% 확률로 스킬 발동 (스킬 발동 시 농작물 한 세트 드롭). 쿨타임: 3분", cost: "170,000,000원" },
  { lv: 170, name: "파브르 Ⅲ", type: "패시브", desc: "수확 시 지렁이 0.3% 확률로 획득 (모든 작물에서 효과 적용)", cost: "200,000,000원" },
  { lv: 180, name: "빛나는 확률 Ⅲ", type: "패시브", desc: "수확 시 산삼 씨앗 0.3% 확률로 획득 (모든 작물에서 효과 적용)", cost: "230,000,000원" },
  { lv: 190, name: "수확신의 축복 Ⅳ", type: "패시브", desc: "수확 시 경험치 12% 추가 획득 (사탕수수에는 작동하지 않음)", cost: "250,000,000원" },
  { lv: 200, name: "드랍더 농작물 Ⅳ", type: "패시브", desc: "수확 시 0.8% 확률로 농작물 추가 드롭 (사탕수수에는 작동하지 않음)", cost: "300,000,000원" },
  { lv: 210, name: "일확천농 [작물] [액티브] Ⅳ", type: "액티브", desc: "수확 시 0.5% 확률로 스킬 발동 (스킬 발동 시 농작물 한 세트 드롭). 쿨타임: 3분", cost: "330,000,000원" },
];
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div>
          ...
          <div className="flex items-center gap-3 flex-wrap mb-2">
            ...
            <h1 style={{ fontSize: "26px", fontWeight: 900, color: "#78350f" }}>🌽 수확 특성</h1>
          </div>
        </div>

        {/* Description */}
        <div className="rounded-2xl border-2 p-5" style={{ background: LIGHT_BG, borderColor: BORDER }}>
          <p style={{ fontSize: "14px", color: "#14532d", lineHeight: 1.8, fontWeight: 600 }}>
          ...
          <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
            <div className="p-2.5 rounded-xl bg-green-100 border border-green-200">
              <p style={{ fontSize: "12px", color: "#166534", fontWeight: 700 }}>
                🚫 도끼·곡괭이로는 작물을 캘 수 없어요. 작물은 <strong>괭이로 캐는 걸</strong> 추천드려요!
              </p>
            </div>
            <div className="p-2.5 rounded-xl bg-orange-100 border border-orange-200">
              <p style={{ fontSize: "12px", color: "#9a3412", fontWeight: 700 }}>
        ...
        <div className="rounded-2xl border-2 border-amber-200 bg-white">
          <div className="px-5 py-4 border-b-2 border-amber-100 flex items-center gap-2">
            ...
            <span style={{ fontSize: "15px", fontWeight: 800, color: "#92400e" }}>자주 묻는 질문</span>
          </div>
          <div className="p-5 space-y-3">
            <div className="p-4 rounded-xl border-2 border-green-100 bg-green-50">
              ...
              <div style={{ fontSize: "13px", color: "#4b7c5e", lineHeight: 1.7 }}>
                A. 수확 특성인 유저분들은 <strong>지렁이</strong>와 <strong>산삼씨앗</strong>을 더 많이 획득할 수 있어요!
                <br />
                <strong>지렁이</strong>는 <strong>섬세한 손길</strong> 인챈트가 있을 때만 드랍돼요.
                추가로, 지렁이의 경우에는 <strong>행운</strong> 수치가 높을수록 확률이 올라가요!
              </div>
            </div>
            <div className="p-4 rounded-xl border-2 border-blue-100 bg-blue-50">
              ...
              <div style={{ fontSize: "13px", color: "#3b5bb5", lineHeight: 1.7 }}>
                A. 수확 특성의 경우, 커스텀 작물 수확 시 특성 경험치를 획득할 수 있습니다.
        ...
        <div className="rounded-2xl border-2 border-amber-200 bg-white">
          ...
          >
            <div className="flex items-center gap-2">
              <span className="text-lg">🐥</span>
              <span style={{ fontSize: "15px", fontWeight: 800, color: "#92400e" }}>커스텀 농사 안내</span>
              <span className="rounded-full px-2 py-0.5" style={{ background: "#fef3c7", color: "#92400e", fontSize: "11px", fontWeight: 700 }}>
                농사 기초 · 도구 · 비료 · 작물
              </span>
            </div>
            {showCustom ? <ChevronUp className="w-5 h-5 text-amber-600" /> : <ChevronDown className="w-5 h-5 text-amber-600" />}
          </button>
            ...
            <div className="border-t-2 border-amber-100 p-5 space-y-5">
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span style={{ fontSize: "14px", fontWeight: 800, color: "#16a34a" }}>커스텀 농사의 기초</span>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  {[
                    ...
                    <div key={item.text} className="p-3 rounded-xl bg-green-50 border-2 border-green-200 flex flex-col items-center justify-center text-center min-h-[80px]">
                      <span className="text-xl mb-1">{item.icon}</span>
                      <p style={{ fontSize: "12px", color: "#166534", lineHeight: 1.5, fontWeight: 600 }}>{item.text}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="p-4 rounded-xl border-2 border-yellow-200 bg-yellow-50">
                ...
                <p style={{ fontSize: "13px", color: "#713f12", lineHeight: 1.6 }}>
                  커스텀 농사를 할 때, 까마귀가 와서 작물을 먹고 도망가요. 그때 허수아비를 설치해놓으면 까마귀를 막을 수 있어요!
                  허수아비의 범위는 <strong>한 청크</strong>입니다.
                </p>
              </div>
              ...
              <div>
                ...
                <p className="mb-3" style={{ fontSize: "13px", color: "#374151", lineHeight: 1.6 }}>
                  커스텀 농작물을 키우기 위해서는 경작지에 물을 뿌려줘야 해요.
                  물뿌리개를 들고 <strong>물에 우클릭</strong>하면 물뿌리개에 물을 채울 수 있어요.
                </p>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  {wateringCans.map((w) => (
                    <div key={w.name} className="p-3 rounded-xl text-center border-2" style={{ borderColor: w.color + "40", background: w.color + "10" }}>
                      <div style={{ fontSize: "12px", fontWeight: 800, color: w.color }}>{w.name}</div>
                      <div className="mt-1 rounded-full px-2 py-0.5 inline-block" style={{ background: w.color + "20", color: w.color, fontSize: "11px", fontWeight: 700 }}>
                        {w.range} 범위
                      </div>
                    </div>
                  ))}
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {sprinklers.map((s) => (
                    <div key={s.name} className="p-3 rounded-xl text-center bg-sky-50 border-2 border-sky-200">
              ...
              <div>
                ...
                <p className="mb-3" style={{ fontSize: "13px", color: "#374151", lineHeight: 1.6 }}>
                  ...
                  비료는 마인크래프트 시간으로 <strong>28일 (약 9시간)</strong>간 유지됩니다.
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  {fertilizers.map((f) => (
                    <div key={f.name} className="p-3 rounded-xl border-2" style={{ borderColor: f.color + "60", background: f.color + "15" }}>
                      <div style={{ fontSize: "13px", fontWeight: 800, color: "#166534" }}>{f.name}</div>
                      <p style={{ fontSize: "11px", color: "#4b7c5e", marginTop: "4px", lineHeight: 1.5 }}>{f.rates}</p>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {customCrops.map((crop) => (
                    <span
                      ...
                      style={{ background: "#f0fdf4", color: "#15803d", fontSize: "12px", fontWeight: 700 }}
                    >
                      🌿 {crop}
                    </span>
                  ))}
