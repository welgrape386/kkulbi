            ...
            {[
              ...
              <div key={s.step} className="flex gap-3 p-4 rounded-2xl bg-amber-50 border border-amber-100">
                <div
                  ...
                  style={{ background: "#f5c842", color: "#1a1200", fontSize: "14px", fontWeight: 900 }}
                >
                  {s.step}
                </div>
                <div>
...
function CommandsContent() {
  const commands = [
    ...
    { cmd: "/상점열기", desc: "개인 상점을 다른 유저에게 공개", cat: "거래", req: "10등급" },
  ];
    ...
    <div className="space-y-4">
      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4">
        <p className="text-amber-800" style={{ fontSize: "13px", lineHeight: 1.7 }}>
          💡 명령어는 채팅창에 입력하면 돼요. 일부 명령어는 특정 랭크 이상이거나 캐시가 필요해요.
          <br />영구 업데이트되는 목록으로, 최신 정보는 디스코드 공지를 확인하세요.
        </p>
      </div>
    ...
    <div className="space-y-4">
      <div className="bg-red-50 border border-red-200 rounded-2xl p-4">
        <p className="text-red-800" style={{ fontSize: "13px", lineHeight: 1.7 }}>
          ⚠️ 규칙 위반 시 경고, 아이템 몰수, 임시/영구 정지 등의 처벌을 받을 수 있어요.
          자세한 처벌 기준은 <Link to="/law" className="underline font-bold">법전</Link>에서 확인하세요.
        </p>
      </div>
...
function PricesContent() {
  const treasureFish = [
    { name: "검정가자미", price: "50만원" },
    { name: "은갈치", price: "75만원" },
    { name: "뚱이", price: "75만원" },
    { name: "푸른바다거북", price: "100만원" },
    { name: "보름달물해파리", price: "120만원" },
    { name: "비단잉어", price: "150만원" },
    { name: "블루랍스타", price: "200만원" },
    { name: "우무문어", price: "200만원" },
    { name: "블로브피쉬", price: "250만원" },
    { name: "바다악어", price: "300만원" },
    { name: "미갈루", price: "400만원" },
    { name: "분홍돌고래", price: "500만원" },
    { name: "백상아리", price: "600만원" },
    { name: "만타가오리", price: "800만원" },
    { name: "고래상어", price: "1000만원" },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-amber-100 rounded-2xl overflow-hidden shadow-sm">
        <div className="px-5 py-4 border-b border-amber-50 flex items-center gap-2">
          ...
          <span className="text-slate-700" style={{ fontSize: "15px", fontWeight: 700 }}>보물 물고기 판매가</span>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-px" style={{ background: "#f5e098" }}>
          {treasureFish.map((f) => (
            <div key={f.name} className="bg-white px-4 py-3">
              <div className="text-slate-700" style={{ fontSize: "13px", fontWeight: 600 }}>🐟 {f.name}</div>
              <div className="text-amber-600" style={{ fontSize: "14px", fontWeight: 800 }}>{f.price}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white border border-amber-100 rounded-2xl p-5 shadow-sm">
        <div className="text-slate-700 mb-3" style={{ fontSize: "15px", fontWeight: 700 }}>📜 낚시대 강화 재료비</div>
        <div className="space-y-2">
          {[
            ...
            <div key={r.to} className="flex items-center gap-2 text-slate-600" style={{ fontSize: "13px" }}>
              <span className="text-slate-400">{r.from}</span>
              <span className="text-amber-400">→</span>
              <span className="text-slate-700 font-semibold">{r.to}</span>
              <span className="ml-auto bg-blue-50 text-blue-700 rounded-lg px-2 py-0.5" style={{ fontSize: "11px" }}>
                🔮 우아한 바다진주 {r.pearl}
              </span>
            </div>
          ))}
